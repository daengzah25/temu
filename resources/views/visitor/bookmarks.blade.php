...
<div class="space-y-4">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-white/60 mt-1">{{ $bookmarks->count() }} UMKM tersimpan</p>
    </div>
  </div>
  ...
  <div class="space-y-4">
      ...
      <div class="bg-white/6 border border-white/10 p-4 rounded-xl">
        <div class="flex gap-3 mb-3">
          ...
          <div class="flex-1 min-w-0">
            <h3 class="font-semibold text-base mb-1 truncate">{{ $company->name }}</h3>
            <p class="text-sm text-white/60 mb-1">
              <span class="inline-block px-2 py-0.5 bg-white/8 rounded text-xs">{{ $company->category }}</span>
            </p>
        ...
        <div class="flex gap-2">
          <a 
            ...
            class="flex-1 px-3 py-2 rounded-lg bg-brand-accent text-black text-sm font-medium hover:bg-brand-accent/90 transition text-center"
          >
            Detail
          </a>
          <a 
            ...
            target="_blank" 
            class="flex-1 px-3 py-2 rounded-lg bg-green-500 hover:bg-green-600 text-white text-sm font-medium transition text-center"
          >
            WhatsApp
          </a>
          <button 
    ...
    @empty
      <div class="text-center py-12">
        ...
        <h3 class="text-lg font-semibold mb-2">Belum Ada Favorit</h3>
        <p class="text-white/60 text-sm mb-6">Simpan UMKM favorit Anda untuk akses cepat</p>
        <a 
...
function toggleBookmark(companyId, button) {
  const card = button.closest('.bg-white\\/6');
  ...
  .then(data => {
    if (data.success) {
      ...
      setTimeout(() => {
        ...
        const remainingCards = document.querySelectorAll('.bg-white\\/6');
